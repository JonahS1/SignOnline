...
var makeIncomingInviteList = () => {
    ...
    .then(querySnapshot => {
        ...
        .then(querySnapshot => {
            querySnapshot.forEach(doc => {
                document.getElementById('inInvList').innerHTML += `<br>${doc.get('senderEmail')}  <i class="material-icons green-text" id="${doc.get('senderEmail')}" onClick='acceptRequestOfEmail("${doc.get('senderEmail')}")'>edit</i>  <i class="material-icons red-text" id="${doc.get('senderEmail')}" onClick='rejectRequestOfEmail("${doc.get('senderEmail')}")'>clear</i>`;
            });
        })
        .catch(function(error){     
            console.error("Error with the QUERY: ", error);
